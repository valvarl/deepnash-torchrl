...
def test_single_trajectory_rollout(env_original: StrategoEnv):
    terminated = False
    for i in range(
        env_original.config.p1_pieces_num.sum()
        + env_original.config.p2_pieces_num.sum()
    ):
...
def test_lose_on_deploy(
    env_original: StrategoEnv, red_surrounded: bool, blue_surrounded: bool
):
    ...
    for i in range(
        ...
        if i < 2 * len(lakes_mask):
            if i % 2 == 0 and red_surrounded or i % 2 != 0 and blue_surrounded:
                state, reward, terminated, truncated, info = env_original.step(
    ...
    if red_surrounded and blue_surrounded or red_surrounded:
        ...
        assert env_original.game_phase == GamePhase.SELECT
        state, reward, terminated, truncated, info = env_original.step(
            env_original.action_space.sample()
        )
        state, reward, terminated, truncated, info = env_original.step(
            env_original.action_space.sample()
        )
