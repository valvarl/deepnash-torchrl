...
class ChasingDetector:
    ...
    def is_on_same_line(self, pos1, pos2, board=None) -> bool:
        if pos1[0] == pos2[0]:
            return (
                board[pos1[0]][min(pos1[1], pos2[1]) + 1 : max(pos1[1], pos2[1])] == 0
            ).all()
        if pos1[1] == pos2[1]:
            return (
                board[:, pos1[1]][min(pos1[0], pos2[0]) + 1 : max(pos1[0], pos2[0])]
                == 0
            ).all()
        return False

    def check_chasing_condition(
        self, verified_piece: Piece, verified_pos: Pos, opponent_pos: Pos, board=None
    ) -> bool:
    ...
    ) -> tuple[bool, list[Pos] | None]:
        if not self.chase_moves or self.chase_moves[-1].attacker:
            return True, None
        forbidden = []
        for i, (chasing_move, chased_move) in enumerate(
            zip(self.chase_moves[::2], self.chase_moves[1::2])
        ):
            if (
                chasing_move.player == player
                and chasing_move.piece == piece
                and chased_move.from_pos == self.chase_moves[-1].to_pos
                and self.check_chasing_condition(piece, pos, chasing_move.to_pos, board)
            ):
                if (len(self.chase_moves) - 1) // 2 == i + 1 and self.chase_moves[
                    -2
                ].from_pos == chasing_move.to_pos:
                    continue
                else:
    ...
    def validate_move(
        self, player: Player, piece: Piece, from_pos: Pos, to_pos: Pos, board=None
    ) -> bool:
    ...
    def update(
        ...
        elif self.chase_moves[-1].attacker and self.check_chasing_condition(
            piece, to_pos, self.chase_moves[-1].to_pos, board
        ):
            # initiative handover
        ...
        if self.check_chasing_condition(
            verified_piece=(
                self.chase_moves[-1].piece if self.chase_moves[-1].attacker else piece
            ),
            ...
            if not self.chase_moves[-1].attacker and not self.validate_move(
                player, piece, from_pos, to_pos, board
            ):
                raise RuntimeError("")
            self.chase_moves.append(
                ChaseEntry(
                    player,
                    piece,
                    from_pos,
                    to_pos,
                    attacker=not self.chase_moves[-1].attacker,
                )
            )
        else:
            self.chase_moves = [ChaseEntry(player, piece, from_pos, to_pos)]
...
class TwoSquareDetector:
    ...
    def validate_move(
        self, player: Player, piece: Piece, from_pos: Pos, to_pos: Pos
    ) -> bool:
        ...
        if to_pos[1 - idx] == start_pos[1 - idx]:
            if (
                start_pos[idx] <= to_pos[idx] <= end_pos[idx]
                or end_pos[idx] <= to_pos[idx] <= start_pos[idx]
            ):
                return False
        return True
    ...
    def update(self, player: Player, piece: Piece, from_pos: Pos, to_pos: Pos):
        ...
        if (
            ...
            and start_pos[idx] <= to_pos[idx] < end_pos[idx]
            or start_pos[idx] > end_pos[idx]
            and end_pos[idx] < to_pos[idx] <= start_pos[idx]
        ):
