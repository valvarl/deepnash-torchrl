    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        sampled_indices_flat = torch.zeros(
            (policy_flat.size(0), 1), dtype=torch.long, device=policy_flat.device
        )
        ...
        if len(valid_indices) > 0:
            ...
            sampled_valid = torch.multinomial(
                valid_policy, num_samples=1, replacement=True
            )
            # shape = (#valid, 1)
        ...
        rows_onehot_padded = torch.nn.functional.pad(
            rows_onehot, (0, max_dim - rows_onehot.shape[1])
        )
        cols_onehot_padded = torch.nn.functional.pad(
            cols_onehot, (0, max_dim - cols_onehot.shape[1])
        )

        # Stack padded one-hot encodings along a new dimension
        action_onehot = torch.stack(
            (rows_onehot_padded, cols_onehot_padded), dim=1
        )  # Shape: (B, 2, max(H, W))

        # Add the one-hot encoded action to the TensorDict
        tensordict["action"] = action_onehot
        tensordict["action_one_hot"] = one_hot_encode(
            sampled_indices, shape[-2] * shape[-1]
        )
