...
bool ChasingDetector::is_on_same_line (const Pos& pos1,
const Pos& pos2,
...
std::pair<bool, std::vector<Pos>> ChasingDetector::validate_select (Player player,
Piece piece,
const Pos& pos,
const std::vector<int8_t>& board,
size_t height,
...
bool ChasingDetector::validate_move (Player player,
Piece piece,
const Pos& from_pos,
const Pos& to_pos,
const std::vector<int8_t>& board,
size_t height,
size_t width) const {
    auto [valid, forbidden] =
    validate_select (player, piece, from_pos, board, height, width);
    if (valid)
        return true;
    return std::find (forbidden.begin (), forbidden.end (), to_pos) == forbidden.end ();
}

void ChasingDetector::update (Player player,
Piece piece,
const Pos& from_pos,
const Pos& to_pos,
const std::vector<int8_t>& board,
...
size_t width) {
    ...
    if (chase_moves_.size () > 1 && chase_moves_[chase_moves_.size () - 2].to_pos != from_pos) {
    ...
    if (check_chasing_condition (
        ...
        chase_moves_.emplace_back (
        player, piece, from_pos, to_pos, !chase_moves_.back ().attacker);
    } else {
        chase_moves_ = { ChaseEntry (player, piece, from_pos, to_pos) };
    }
}
...
const std::vector<std::pair<Pos, Pos>>& TwoSquareDetector::get_player_moves (
Player player) const {
...
bool TwoSquareDetector::validate_move (Player player,
Piece piece,
const Pos& from_pos,
const Pos& to_pos) const {
...
void TwoSquareDetector::update (Player player, Piece piece, const Pos& from_pos, const Pos& to_pos) {
    ...
    if ((start_pos[idx] < end_pos[idx] && start_pos[idx] <= to_pos[idx] &&
