...
private:
    ...
    Matrix<bool> lakes_mask_;
    Matrix<bool> p1_deploy_mask_;
    Matrix<bool> p2_deploy_mask_;
    ...
    Matrix<bool> make_mask(const std::vector<std::pair<Pos, Pos>>& positions) const;
...
public:
    StrategoConfig(
        ...
        const Matrix<bool>& lakes_mask = Matrix<bool>(0, 0),
        const Matrix<bool>& p1_deploy_mask = Matrix<bool>(0, 0),
        const Matrix<bool>& p2_deploy_mask = Matrix<bool>(0, 0),
    ...
    const Matrix<bool>& lakes_mask() const { return lakes_mask_; }
    const Matrix<bool>& p1_deploy_mask() const { return p1_deploy_mask_; }
    const Matrix<bool>& p2_deploy_mask() const { return p2_deploy_mask_; }
    ...
    StrategoConfig rot90(int k = 1) const;
};
