...
class StrategoConfig {
    ...
    size_t total_moves_limit () const {
        return total_moves_limit_;
    }
    size_t moves_since_attack_limit () const {
        return moves_since_attack_limit_;
    }
    size_t observed_history_entries () const {
        return observed_history_entries_;
    }
    const std::vector<int>& allowed_pieces () const {
        return allowed_pieces_;
    }
    const std::vector<bool>& lakes_mask () const {
        return lakes_mask_;
    }
    const std::vector<bool>& p1_deploy_mask () const {
        return p1_deploy_mask_;
    }
    const std::vector<bool>& p2_deploy_mask () const {
        return p2_deploy_mask_;
    }
    const std::unordered_map<Piece, int>& p1_pieces () const {
        return p1_pieces_;
    }
    const std::unordered_map<Piece, int>& p2_pieces () const {
        return p2_pieces_;
    }
