...
PYBIND11_MODULE(stratego_cpp, m) {
    ...
    .def("step", [](StrategoEnv& env, const Pos& action) {
        ...
        py::array_t<double> obs_array({obs.size() / height / width, height, width}, obs.data());
    ...
    .def("get_info", [](StrategoEnv& env) {
        ...
        py::array_t<int> allowed_pieces_array(allowed_pieces.size(), allowed_pieces.data());
